...
function returnCheckLogin($login, $id){
  ...
  $check_login_user->bindValue('login', Chiffrement::crypt($login), PDO::PARAM_STR);
  $check_login_user->bindValue('id', $id, PDO::PARAM_INT);
...
function editContactMessageStatut($id, $statut){
  ...
  try {
    ...
    WHERE id LIKE :id AND request_closed LIKE (0 OR 1)';
